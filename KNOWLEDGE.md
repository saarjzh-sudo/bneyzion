...
## סשן 15.7.2026 — רמה 18: סבב הערות הרב יואב (ערב 14.7) + אודיט תרומות/הקדשות/אדמין

**מקור:** כל הערות יואב בוואטסאפ מערב 14.7 (טקסט + 5 תמונות + 3 הקלטות שתומללו ב-Groq Whisper) + אודיט צ'קליסט יזום לפי בקשת סער ("שהכל יעמוד בסטנדרט" לפני שיואב עובר על אגף התרומות).

### חנות
- **עורך תוכן ויזואלי** למוצרים (Products) ולשיעורי-קורס (CommunityCourses) — RichTextEditor (TipTap, קיים משיעורים) במקום Textarea של HTML גולמי. הערת יואב 21:23.
- **מחיקת מוצר עם רכישות** → ארכוב רך (pre-check על order_items, כמו מחיקת-רב 16.5). הערה 21:27.
- **נקודות-איסוף ברכישת-עגלה** — usePublicSalePoints חובר גם ל-Checkout.tsx (היה רק בקנייה-מהירה; זו הסיבה ש"אצלינו בבית" לא הופיעה ליואב). הערה 21:39.
- **יישור שני מסלולי הצ'קאאוט** (הערה 10:11): עגלה = שם פרטי+משפחה (כמו המודל), קנייה-מהירה קיבלה בחירת תשלומים (עד 12).
- **ניקוי ג'יבריש**: 5 מוצרים (wc-687/695/2523/2527/2530) עם nav+מחירים ישנים+svg ב-description+content → NULL (snapshot `products_bak_junkclean_20260715`). ה-X הענק = svg מהאתר הישן; sanitizeHtml עכשיו חוסם svg גלובלית (FORBID_TAGS). **מקור הזיהום: enrich-products** — ה-fallback השלישי גורף תפריט; נוסף שומר-ג'יבריש (JUNK_MARKERS) + סינון svg, נפרס v10.
- **איחוד ניהול משלוחים**: ShippingOptionsEditor (קומפוננטה חדשה) בטאב "משלוח ואיסוף" בניהול-מוצרים לצד נקודות-המכירה; הוסר ממרכז-השליטה (נשארה הפניה). הערה 21:42.
- **תיאורי קטגוריות גלויים** (הערה 21:11): כותרת+תיאור בכניסה לקטגוריה + תיאור על אריחי הקטלוג + title על כפתורי הפילטר.
- **בידול "קובצי הספרים שבתהליכי כתיבה"** (אודיו 08:49): כרטיס ענבר עם מסגרת מקווקוות + badge "בתהליך כתיבה" (slug=upcoming) בחנות ובעמוד המוצר.
- **מסירה דיגיטלית**: "הקובץ אישי — נא לא להעביר" במייל ובדף-התודה (אודיו 08:49).

### קורסים (הערות 22:10 + אודיו 22:21/22:46)
- **שורש "לא מצאתי איפה עורכים"**: העריכה קיימת אבל נפתחת רק בלחיצה על גוף כרטיס-הקורס. נוסף כפתור גלוי-תמיד "עריכת פרקים, שיעורים ותכנים" על כל כרטיס.
- **מעקב התקדמות לומד** (הבסיס של כל LMS שיואב ביקש): טבלה חדשה `course_lesson_progress` (RLS per-user, מיגרציה 20260715) + hook useCourseProgress + סימוני ✓ "נלמד" ופס-התקדמות ב-WeeklyBookDetail (כל שכבות הפרקים+הקדמה+תכנים-נוספים) וב-CommunityCoursePage. ⚠️ MediaCard שוחזר כ-פקד+כפתור אחים (לא פקד בתוך פקד — נגישות).

### ניווט — "נהר התנ״ך" (river.html של יואב, אודיו 18:03/19:32)
- קומפוננטה חדשה `src/components/bible/BibleRiver.tsx` — פורט React של האב-טיפוס: נהר חטיבות ← רצועת ספרים (רוחב∝פרקים) ← רצועת פרקים/פרשות (תורה+יהושע=יחידות). הוטמעה בראש `/bible` (ההחלטה: בעמוד הניווט, לא בדף הבית); רשימת הכפתורים נשארה מתחת כ-fallback סטטי (SEO+נגישות). state ב-URL (?section&book). BibleBookPage קורא `?chapter=N` ופותח את הפרק.
- טקסטורות אקוורל אמיתיות — כשיופקו (כרגע גרדיאנטים בשפת-הצבע).

### תרומות והקדשות (אודיט לקראת הסבב של יואב)
- **בידול ב-Grow** (תלונת 21:01): DonateForm בונה description עם שם-מקור ("תרומה — קמפיין ספר יהושע"); דף /donate קורא ?source/tier/amount (קודם התעלם — fallback מיהושע איבד ייחוס וסכום).
- **מייל תודה לתורם** — sendDonationThankYouEmail (Smoove single-recipient) נשלח מה-webhook על תרומה שאושרה; חיובי-המשך של הו"ק לא מגיעים לשם (grow-webhook-sync) אז אין ספאם חודשי.
- **כשלי חיוב הו"ק גלויים**: FailedRecurringCard בטאב התרומות (grow_webhook_log, action=recurring-charge-failed%, admin-read RLS קיים). קודם — תורם שנפל היה בלתי-נראה.
- **הקדשות**: ייצוא CSV + כל התמחור הדיפרנציאלי (רב-מבוקש/ספי-סדרה) עריך בדיאלוג המחירים (השרת כבר אכף אותם, האדמין לא חשף) + התראת-משרד על כל הקדשה חדשה שעולה לאתר (השם עולה אוטומטית — עכשיו יש עין).

### אדמין — ניקיון רוחבי
- סיידבר: נוספו "הזמנות ומשלוחים" (/admin/orders — היה יתום גמור!) ו"הידע של בנצי" (/admin/benzi).
- הוסרו: HomepageManager (דד-רייט — כתב homepage_config שאף רכיב לא קורא) + ראוט; ContentHealth/ImportContent/Users.tsx (0 refs); SmooveImportDialog מ-Subscribers (כפתור-מת + "288" קשיח); lazy-import מת של Donate הישן (הקובץ נשאר כעותק-חזרה מכוון).
- שומרי-RLS (.select+בדיקת שורות) למחיקות: Coupons (גם onError — הייתה מחיקה כושלת-שקטה), Promos, CommunityCourses (קורס+שיעור+סטטוס), useEvents/useLessons/useSeries/useTopics.
- Dashboard: תגית מקור "Grow" תוקנה ל"הזמנות" (המספר מגיע מ-orders).

### מלכודות שנלמדו
- **enrich-products היה מקור זיהום-חוזר**: ניקוי DB בלי לחסן את הפונקציה = הג'יבריש חוזר בלחיצה הבאה. תמיד לתקן את המקור.
- **הכנסת guard בסקריפט לפי "ה-throw הבא" מסוכנת** — ב-CommunityCourses ה-guard נחת בתוך mutation אחר (deleteCourse בלי `if (error) throw` צמוד). לאמת כל edit ממוכן.
- dedication_settings: עמודות DB הן price_lesson/price_series (המיפוי בשם-הפוך נעשה ב-useLessonDedications).

**מחסומים:** tsc נקי · build נקי · regression_guard נקי ("רמת מיגרציה 8.5 נשמרת").

### תוספת 15.7 (המשך רמה 18) — שני מבני-קורס + אימות מנגנון הפרק-השבועי
- **הפיצוח (סער):** ספרי הפרק-השבועי = פרק + שלוש שכבות (בסיס/הרחבה/שיעור — כמו בשטח); קורס רגיל = `section_title` חופשי (נושא/פרק) ← שיעורים. LessonForm מסתעף לפי `isProgramBook` (program_slug/in_weekly_program); הלומד: CommunityCoursePage מקובץ לפי נושאים.
- **אימות מנגנון:** הפרק הנוכחי = ההקלטה (weekly) האחרונה בספר is_current — עובד; כרגע זכריה פרק ב'. **גילוי: זכריה יובא חסר** — רק intro+weekly, בלי base/enrichment (עזרא = הרפרנס המלא); פריטים פרקיים (דף-הכוונה/תרשים) תקועים ב-intro בלי פרק. להשלים מהמקור.
- **דאטה:** נוקו 184 תיאורי "פרק N" בספרות (snapshot `ccl_bak_desc_20260715`); "שיעורי הרב ברוך סליי — פרק N"→אות עברית; באדמין פרקים באות עברית (hebChapter).
- **קורס לדוגמה** "קורס לדוגמה — מבנה קורס רגיל" נשאר ב-DB (draft, מוסתר מקטלוג) להמחשת המבנה. משתמש-הבדיקה שנוצר לצילום נמחק (auth+tag).
- נשלח ליואב: הסבר שני הסוגים + 2 צילומי מסך + המלצת-סער להסתיר את קורסי-הספר הנמכרים עד הכרעה אסטרטגית (לא הוסתרו — ממתין להחלטה). אומת delivered.

### תיקון-כיוון 15.7 (סער) — קורס-ספר נמכר ≠ הפרק-השבועי, גם על אותו ספר
- **ההבחנה:** ספר עזרא בפרק-השבועי (מנוי) = פרק+3 שכבות. **קורס עזרא הנמכר** (רוכש בודד) = מוצר אחר: קורס רגיל שמבוסס על אותם תכנים — נושאים בשם חופשי, שיעורים ממוספרים, עריכה פשוטה (בסגנון קורסי אבולעפיה/סקולר של סער).
- **כלי:** `scripts/bz_make_book_course.py book-<slug>` — מעתיק (לא מזיז) ספר שלם לקורס נמכר: "פתיחה והקדמה" + נושא לכל פרק ("פרק א׳ — הצהרת כורש"), סדר בסיס→הרחבה→שיעור. אידמפוטנטי (--refresh). מזהה: course_type='standalone-book', access_tag='course-<book>'.
- **פיילוט:** "קורס ספר עזרא" `fa23abe0-d580-4fa8-a9aa-98062e7e41d2` — 84 שיעורים / 11 נושאים, draft (מוסתר עד הכרעה אסטרטגית).
- ⏳ **חיווט בהמתנה להכרעה:** רכישת book-* עדיין מובילה ל-WeeklyBookDetail; כשמפעילים מכירה — להפנות רוכשים ל-/portal/course/<הקורס-הנגזר> ולהעניק תג לפי access_tag החדש.
- ⚠️ **לקח תהליכי (נשמר גם ב-memory):** שום שליחה ללקוח בלי להראות לסער קודם — גם אם ההוראה המקורית אמרה "תשלח לו".

### סבב-עיצוב 15.7 — נגן הקורס הרגיל (CommunityCoursePage) בסגנון סקולר
- הערת-סער על הפיילוט: "הייתי משקיע יותר בעיצוב — ההדר של האתר למעלה, סיידבר של השיעורים, שהראשון ייפתח כמו סקולר".
- שוכתב CommunityCoursePage: DesignLayout (הדר האתר) → הירו-קורס (mahoganyHero, תמונה, badge שיעורים/נושאים, פס-התקדמות) → גריד: סיידבר "תכנית הלימודים" (נושאים מתקפלים, הראשון פתוח, ✓ + אייקון-מדיה לכל שיעור, sticky) + נגן מרכזי (וידאו/שמע/קובץ-מוטמע/טקסט, "סמן שסיימתי", הקודם/הבא). מובייל: נגן קודם, תכנית אחריו. הדיאלוג הישן הוסר.
- פרוס `8725c20d`; צילום נשלח לסער לביקורת (לא ליואב).

---

## רמה 21 — הערות-בוקר סער 17.7 (self-chat WA)

**מקור:** שתי הודעות בצ'אט-העצמי של סער 17.7 09:23. **commit:** `ba33e19c` · **tag:** `level21-2026-07-17` · **prod:** `dpl_GJZpfReTEuF1yh4UhhkQyWjqvCp7` ← alias `bneyzion.vercel.app`. **rollback: alias ל-`dpl_7pB9QGnwHP8xeWSuvjs39Gr5yAC9`.** דאטה לא שונה.

1. **/kenes באנר קריא:** ה-hero הישן = `font-kedem-hollow` (משקל 100) בגרדיאנט ישירות על צילום חומות-ירושלים, בלי overlay — בלתי-קריא. הותאם לדפוס של שבועות/מלחמות-התנ״ך: overlay קרם עדין + כרטיס `bg-[hsl(38_50%_94%/0.90)] backdrop-blur rounded-3xl` + `font-kedem-hollow-aaa text-black`. CTA יחיד (הקלטות); כפתור קהילת-הווצאפ בהירו הוסר יחד עם ההפניה לקבוצות.
2. **הפניית-קבוצות בתחתית /kenes הוסרה:** כל סקשן "הצטרפו לקהילה שלנו" (2 מוקאפי-טלפון: פסוק-יומי + קריאת-כיוון, כפתורי יוטיוב/ספוטיפיי/WA) נמחק כולל `PhoneMockup` והנכסים. הדף מסתיים ב-CTA תרומות → פוטר.
3. **כנסים תחת "ימי עיון בתנ"ך":** `src/data/kenesim.ts` = מקור-אמת יחיד (3 כנסים), משרת את `/kenes-archive` + בלוק "כנסי בני ציון" חדש ב-`CategoryPage` כש-`id === f4040001-0001-4000-8000-000000000000`. כנס חדש → שורה אחת ב-kenesim.ts מופיעה בשני המקומות.
4. **/bible בלי נהר-התנ״ך:** `BibleRiver` (אב-טיפוס הרב יואב, רמה 18 — יואב ביקש, סער הוריד 17.7) הוסר מ-BibleIndexPage + כותרת-הביניים "כל הספרים — רשימה מלאה" הוסרה. הרשימה המלאה = הניווט. הקומפוננטה `src/components/bible/BibleRiver.tsx` נשארה בקוד (לא בשימוש).
...
## 3.9.2026 — עומס Supabase אחרי מעבר הדומיין: הפולינג עבר ל-react-query (סער: "תתקן את הפולינג")

1. **הממצא:** מ-28.8 (הדומיין על האתר החדש) בקשות ה-REST ל-Supabase קפצו מ-~490/שעה ל-~7,650/שעה (185-208K ליום, 1.02M בשבוע), עם רצפה של 2-4K/שעה ב-03:00. ה-DB (Micro, 2 ליבות משותפות, 1GB) מחזיק — cache-hit 100%, 28/60 חיבורים — אבל `campaign_stats` (0.6ms ב-EXPLAIN) חזר ב-2.5ש' ממוצע = תור, לא שאילתה. **Vercel לא בתמונה** (SPA סטטי, `x-vercel-cache: HIT`; הבקשות הולכות ישר ל-Supabase).
2. **השורש:** `CampaignBanner` (בכל דף) + `HomeCampaignStrip` + `CampaignPage` קראו כל אחד ל-`useLiveCampaignStats` עם `setInterval(30_000)` משלו — גם בטאב ברקע. ה-realtime על `donations` לא יורה לגולשים (RLS `admin_select_donations` = 3 מיילים), אז הפולינג היה המסלול היחיד. ב-`useParasha` 4 שאילתות עם `refetchInterval: 30_000` (כל טיק 25-35 קריאות) **בנוסף** ל-realtime על `lessons`/`series` — שכן בפרסום `supabase_realtime` ועם `anon_read`, כלומר ה-realtime שם כן עובד.
3. **התיקון (2 קבצים):** `useLiveCampaignStats`/`useLiveTierCounts` → react-query עם queryKey משותף (`["campaign-stats", slug]`) — קריאה אחת לכל ה-consumers, `refetchInterval` 60ש' רק בטאב גלוי, `refetchOnWindowFocus`, ה-realtime מבטל מטמון; `useParasha` → `PARASHA_FALLBACK_POLL_MS = 5 דק'` (רשת-ביטחון בלבד). `useTierCounts.ts` (רק `/design-yehoshua-campaign`) ו-`DesignPreviewYehoshuaAdmin` (45ש', אדמין) לא נגעתי.
4. **אימות:** `tsc -b && vite build` עובר. מדידה ב-`performance.getEntriesByType('resource')` בטאב מוסתר (= טאב ברקע אצל גולש): פרודקשן `/campaign/saadia` 9 בקשות/91ש' (3×campaign_stats + 3×campaign_tier_counts), הבאנדל המתוקן 5/131ש' (0 פולינג ברקע); בית 69/90ש' → 64/112ש'. הדף מציג ₪293,447/₪350,000, בלי שגיאות קונסול.
5. **🧠 מלכודות:** (א) `preview_start` על ה-worktree נופל ב-`EPERM` על `node_modules` תחת `~/Downloads` (TCC; node לא, python3 כן) → בונים ב-Bash, מעתיקים `dist/` לסקראצ'פד ומגישים עם `spa_server.py` (launch `bneyzion-dist`). (ב) חלון-הדפדפן של קלוד מדווח `visibilityState: hidden` → react-query משהה intervals; `setInterval` גולמי לא — זה בדיוק ההבדל שנמדד. (ג) `/v1/usage` של Vercel דוחה כל טווח עם הטוקן — מדדי-שימוש רק בדשבורד.
6. **הצעד הבא (לא בוצע, דורש אישור נפרד):** 649MB ב-147 טבלאות `*_bak*` מתוך 809MB; אינדקס על `donations(product, payment_status)` ל-views של הקמפיין; `@vercel/analytics` לא מותקן בקוד למרות ש-Web Analytics דלוק בפרויקט.

### פריסה + אימות חי (3.9.2026, סער: "פרוס")

- `a3b39a5c` נדחף ל-main; דיפלוי פרודקשן **READY 12:41:39** (`bneyzion-9glfjqp18`). הדומיין מגיש `main-DiZbLgp5.js`.
- **אימות בבאנדל החי (לא בקוד המקומי):** `useParasha-Cq4SKXLL.js` → `w=5*6e4` (5 דק') ב-4 ה-queries; `main` → `dM=6e4` (60ש') לקמפיין; `["campaign-stats"]`/`["campaign-tier-counts"]` כ-queryKey; **0 מופעים של `setInterval(…,3e4)`** בכל הבאנדל. הבנייה המקומית (`main-BhSsL4np`) יוצאת בהאש אחר מהבנייה של Vercel (node 26 מול 24.x) — משווים סמנים, לא האש.
- **מדידה חיה** על `bneyzion.co.il/campaign/saadia` אחרי `unregister()` של ה-SW וניקוי caches: 5 בקשות REST ב-101ש', **0 פולינג ברקע** (מול 9 ב-91ש' בבאנדל הקודם `bneyzion-a6igu5hr0`). בלי שגיאות קונסול.
- ⚠️ **התראת-שווא שנחקרה עד הסוף:** בדף הקמפיין הופיע "₪0" במקום סכום הגיוס. **לא רגרסיה** — `useCountUp` (CampaignPage.tsx:168) בנוי על `requestAnimationFrame`, ובפאנל-הדפדפן של קלוד rAF יורה **0 פעמים** (נמדד; `visibilityState` תמיד `hidden` גם בטאב שנבחר). פס ההתקדמות, שנרנדר ישירות מ-`totalRaised` בלי rAF, הראה **84%** = 4,947+290,142 מתוך 350,000 — כלומר `raised` הגיע לקומפוננטה כשורה. הבאנדל הישן הציג ₪295,139 רק כי נטען ברגע שהפאנל היה גלוי. **לקח: כל מספר מונפש-rAF באתר אינו ניתן לאימות מהפאנל — לאמת דרך אלמנט לא-מונפש (פס/aria) או בדפדפן אמיתי.**

## 3.9.2026 — סבב עדינות לדף קמפיין סעדיה + הקדשה-לפי-שם באדמין (בקשות סער + הרב יואב במסתערים)

1. **קיר תורמים "השותפים שלנו"** ב-CampaignPage (אחרי החבילות): שמות/סכומים/ברכות מ-`campaigns.external_recent_donors` (עמודת jsonb חדשה) — הדאטה הציבורי של עמוד givechak, מסונכרן ב-`sync-givechak-saadia.py` (וואן-מן-שואו): דפדוף עם `last_oid` (12 לעמוד), עד 48 תורמים, שער fail-closed. **תורמים מקומיים לא נחשפים** (לא נתנו הסכמת-פרסום כמו בגיבצ'ק). רשימה ריקה (יהושע) = הסקשן לא מרונדר.
2. **פס-תרומה תחתון דק** (`StickyDonateBar`): fixed בתחתית, מופיע אחרי scrollY > innerHeight*0.75, "נשארו ₪X" + כפתור + "מוכר לסעיף 46" (מוסתר <380px). הפוטר קיבל כפתור-תרומה + סעיף 46 ו-padding תחתון 92px שהפס לא יכסה אותו.
3. **גרדיאנט נודד על החלק שחסר** בפס-ההתקדמות של ההירו (`.campaign-remaining-shimmer`, keyframes ב-<style> של הדף, prefers-reduced-motion מכובד) + כיתוב "נשארו ₪X להשלמת היעד".
4. **הקדשה ידנית באדמין (בקשת יואב 3.9 במסתערים, תורם ₪1,000 לע"נ):** דיאלוג ה-seeding ב-Dedications.tsx עבר מבחירת UUID לחיפוש-לפי-שם — שימוש חוזר ב-hooks של הבוחר הציבורי (`useDedicationLessonSearch`/`SeriesSearch`/`TakenIds`), תג "כבר יש הקדשה" (עדיין ניתן לבחירה — ריבוי הקדשות נתמך ב-DB), והדבקת UUID נשארה כנתיב-חירום.
5. **🧠 קריאת מסתערים כשקווי-סער מתים:** קו ידידיה (7105593556) חבר בקבוצת מסתערים — getChatHistory משם עובד. מדיה נכנסת הורדה מיד (פגה).

## 14.9.2026 — גישת העלאה נוחה לאצוות אגף המורים (בקשת הרב יואב; סער: "תייצר ותפרוס")

1. **הבקשה (וואטסאפ 20:24):** יואב מעלה אצוות חוברות לאגף המורים (מורשה לדברים + 21 חוברות של הרב גדי שר שלום) — ~15 פעולות לחוברת, בשתי פעימות, בגלל: (א) בורר-רב שטוח של 212 שמות בלי חיפוש בטופס "שיעור חדש"; (ב) האשף לא מציג סדרות "מורים". הוא הציע שלוש אופציות (service key / מפתח מצומצם / תיקוני-אדמין) — נבחרה ג': תיקון מוצר בלי מסירת מפתח (תיקון-קוד עדיף על פתיחת-הרשאות).
2. **RabbiCombobox חדש** (`components/admin/RabbiCombobox.tsx`, תבנית TopicCombobox, סינון-לקוח) מחליף את ה-Select השטוח ב-Lessons.tsx וב-Series.tsx; אגב כך נוסף "ללא רב" בדיאלוג הסדרות (לא היה ניתן לנקות רב שם).
3. **האשף מציג סדרות-מורים בכל צומת:** `useSeriesForNode` קיבל `includeTeachers` (ברירת-מחדל false — הדפים הציבוריים לא השתנו); ה-NodeSeriesPanel באשף קורא עם true ומצייר תג "מורים" על סדרות teachers-בלי-general. **ממצא-מפתח:** סדרות המורים מפוצלות בין העצים — "חוברות ת"ת מורשה במדבר/שמות" תחת מאגר-עזרי-הלמידה, אבל "חוברות מורשה דברים" ו"חוברות הרב גדי במדבר" תחת ספרי התורה **הציבוריים** (`bb14b5a5`) — לכן ההכללה בכל צומת, לא רק בלשונית החדשה.
4. **לשונית "אגף המורים"** ב-ContentLocationPicker: `useTeachersMaagarTree` (שני מפלסים תחת השורש `6bfb7aaa-cd9e-4562-b087-a37fcc24d295`; תורה→נביאים→כתובים בסדר קאנוני, השאר א"ב) + שורת "כל האגף" לסדרות שישירות תחת השורש; חיפוש-המיקום (Feature 4) מכסה גם את העץ הזה.
5. **תיוג אוטומטי:** בחירת סדרת-מורים או צומת-מאגר (SelectedLocation.wing="teachers") מחליפה את ברירת-המחדל ["general"] ב-["teachers"] עם toast; בחירת-קהל מותאמת של המשתמש לא נדרסת.
6. **זרימת אצווה:** מסך-הסיום קיבל "העלה עוד לאותה סדרה" (`resetKeepAssignment`) — שומר רב/סדרה/קהל-יעד/ספר, מנקה כותרת/תיאור/מדיה/נושאים/פרק; סדרה שנוצרה-הרגע מומרת ל"סדרה קיימת" בבחירה כדי שלא תיווצר כפולה.
7. **אימות:** `tsc -b` + `vite build` נקיים; המחרוזות החדשות בבאנדל (`ContentUpload-*`, `RabbiCombobox-*`). אימות ויזואלי מלא של /admin דורש חשבון אדמין (Google OAuth) — נבדק ברמת באנדל-חי + טעינת דף ציבורי.

## 15.9.2026 לפנות בוקר — דור הפלאות: שדרוג פי 10 לדף הקמפיין + מצב-מוצר גנרי (פרוס, `2e10ccf3`)

בקשת סער ("שפר פי 10... תגנרט עם גימיני מה שנצרך"; אישור-דיפלוי: "יאלה פרוס ושלח לי שאתה מסים"). סשן בנצי.

1. **עמודת `campaigns.is_product` (DDL אדיטיבי, default false)** — קמפיין-מוצר לעומת קמפיין-תרומה. הגרנטים על הטבלה הם ברמת-טבלה (אומת מול information_schema), לכן עמודה חדשה לא שוברת `select("*")` לאנון. snapshot לפני: `campaigns_bak_20260915_dor` + `campaign_tiers_bak_20260915_dor` (שתיהן עם RLS-בלי-פוליסי = חסומות לאנון).
2. **מצב-מוצר ב-CampaignPage.tsx:** בלי בלוק "הושג עד כה ₪0" בהירו · CTA-ים "להזמנת החוברת"/"להזמנה" (הירו, כרטיסי-חבילות, פס-תחתון, פוטר, FinalCTA) · כותרת החבילות "כמה תיקחו הביתה?" · בלי DonorPulseToasts · "0 תומכים" לא מוצג (הזמנות מוצגות רק כש>0, בתווית "הזמנות כבר נקלטו") · **בלי אזכורי סעיף 46** (רכישת מוצר אינה תרומה מוכרת-מס — הוסתר בפס-הדביק ובפוטר) · תיאור החיוב ל-Grow: "הזמנה — דור הפלאות" · placeholder ת"ז נוסח-קבלה. `StickyNav` התגלה כקוד-מת (מוגדר, לא מרונדר). קמפיינים קיימים לא הושפעו — fingerprint (md5 של story+hero+is_product) של saadia/yehoshua זהה לפני/אחרי.
3. **6 תמונות Nano Banana Pro (gemini-3-pro-image, 2K)** לפי השיטה הנעולה של סדרה 2 (כריכת הדפוס האמיתית 1004×1418 כ-inline_data + סעיף FIDELITY + בלי אנשים/פרצופים): hero-v2 (16:9 כהה-קולנועי), story-shabbat (שולחן שבת), tier-1/2/5/10 (1:1, ריבועי — כרטיסי החבילות הם aspect-ratio 1:1). ⚠️ לקח סקריפטינג: base64 של הכריכה (~1.8MB) כ-argv מפוצץ ARG_MAX במאק — לבנות request-file בפייתון, לא להעביר כארגומנט. הנאמנות אומתה חזותית פר-תמונה (כולל זום על שורת "יואב אוריאל" בהירו). Storage: `lesson-files/dor-haplaot-campaign/v2/*.webp` (נתיבים חדשים, לא דריסה; אומתו 200 לפני עדכון הרשומות).
4. **תוכן (DB-only):** story_html מלא מהקופי המאושר של 8.9 (הוקים, פסוק תהלים עח,ד "לְדוֹר אַחֲרוֹן מְסַפְּרִים תְּהִלּוֹת ה'..." — מקור שם החוברת, גלריית 4 עמודי-הדפוס, רשת 10 הפרקים, קופסאות מוסדות/מפיצים נשמרו) · why_cards ×3 · faq ×5 (רק עובדות שהוכרעו — משלוח כלול מ-5+, איסוף עצמי ל-1-2) · author = הרב יואב אוריאל (הביו מבוסס על הנוסח המאושר מקמפיין יהושע; בלי תמונה — אין תצלום אמיתי ב-assets, ואסור לג'נרט דמות אמיתית) · hero_subtitle מגרסת-ההירו שאושרה בביקורת 8.9.
5. **אימות:** dump-dom חי — "כמה תיקחו הביתה" ✓, "להזמנת החוברת" ×4 ✓, אפס שפת-תרומה (ה"סעיף 46" היחיד = קומנט CSS) · צילומי דסקטופ+מובייל חיים. ⚠️ צילום headless של SPA: `--virtual-time-budget` עלול לצלם לפני ה-hydration — dump-dom הוא האימות האמין, צילום שני עם budget גבוה יותר הצליח.
6. **נשאר פתוח:** מחירי המדרגות + מוסדות = טיוטה עד אישור יואב · הדף עדיין סמוי (בלי באנר, לא מקושר) · פליירים/קופי-קבוצות אצל סער לאישור (ביטלי bit.ly/4rlPdl1 → /campaign/dor-haplaot?src=wa-groups) · פער 108 (קופי) מול 114 (עמודי קובץ-הדפוס) — לוודא מול יואב.

### 15.9.2026 בוקר — סבב-המשך דור הפלאות: הירו-נייד, שורת-מספרים ואייקונים (`1c3d4acb`+`6eaf78fe`, פרוס)

הערות סער בוואטסאפ אחרי הסבב הראשון:
1. **הירו-נייד במצב-מוצר:** eyebrow קטן ("במלאות 3 שנים למלחמה", עודכן ב-DB) → ההדמיה בשלמותה כתמונת-בלוק (`campaign-hero-inline-img`, מוצגת רק ב-≤767px) → כותרת/סאבטייטל/CTA. תמונת-הרקע (`campaign-hero-bgimg`) מוסתרת בנייד; בדסקטופ הכל כמו שהיה.
2. **סטריפ-המספרים בנייד = שורה אחת תמיד:** `.campaign-proof-grid` הופך ל-flex עם ילדים `flex:1 1 0; min-width:0` (basis-0 = שלישים שווים בלי רצפת min-content).
3. **אייקונים איכותיים במקום אימוג'י (כלל גורף חדש של סער לכל דף):** `STAT_ICONS` הורחבה — ✨→Sparkles, 🇮🇱→Flag — כך שכל proof_stat מרונדר כאייקון lucide בעיגול זהב. נשמר זיכרון גלובלי `feedback_no_emoji_icons_use_quality_vector_icons`.
4. **⚠️ לקח-אימות קריטי:** צילום `--headless=new --screenshot --window-size=390,...` **בלי** אמולציית-מובייל מייצר לייאאוט שקרי (נראה כאילו התוכן נחתך בקצה הימני — "הצפה" שלא קיימת). אימות-מובייל אמין = CDP עם `Emulation.setDeviceMetricsOverride {mobile:true}` (+`suppress_origin=True` ב-websocket-client, אחרת 403), ואז `Page.captureScreenshot {captureBeyondViewport:true}`. נמדד בפועל: scrollWidth=390=viewport, אפס חורגים אמיתיים.
5. `StickyNav` ב-CampaignPage = קוד-מת (מוגדר ולא מרונדר) — קיבל תמיכת isProduct ליום שיחובר.

### 15.9.2026 צהריים — דור הפלאות: סבב תיקוני הרב יואב (2 הקלטות + מסתערים) — `5f2777e0`, פרוס ואומת

הרב יואב שלח 2 הקלטות לפרטי של סער + הערות במסתערים ("העמוד ממש יפה. יש שם הרבה ביטויים ובעיות של בינה מלאכותית"; "הסקשן הראשון כהה מדי"). תומללו ב-whisper-cli מקומי. מה הוטמע:
1. **הירו בהיר במצב-מוצר, בכל הרזולוציות** (קוד): רקע קרם, ההדמיה כבלוק מלא (max‑width 780), טקסט נייבי, eyebrow כגלולת-נייבי — במקום ההירו הכהה שהטקסט שלו ישב על כיתוב הכריכה. תמונת hero חדשה בהירה (`v2/hero-bright.webp`, nano-banana מהכריכה).
2. **הפסוק תוקן** — ציטטתי קודם קטע אמצע-פסוק שמשנה משמעות ("לדור אחרון מספרים…"); הוחלף בפסוק המלא "לֹא נְכַחֵד מִבְּנֵיהֶם…" (תהלים עח, ד). ⚠️ לקח: פסוק לדף = תמיד הפסוק השלם, לא פרגמנט "יפה".
3. **"איור אקוורל" → "איור צבעי מים"** בכל הדף (הקהל לא מכיר אקוורל).
4. **שכתוב אנושי** של הסיפור, why_cards, faq והסוגר; הסוגר החדש (ניסוח יואב): "ניסי התנ"ך מופיעים מחדש, בדור שלנו" — מוזן מ-`hero_quote`/`hero_quote_cite` שה-FinalCTA במצב-מוצר קורא (שדות שהיו dead-DB).
5. **"מי מאחורי הקמפיין" → "מי מאחורי החוברת"** במצב-מוצר (קוד).
6. **מחירים:** tier-5 ‏180→200 (הוראה מפורשת), tier-10 ‏300→340 (הוראתו "להעלות בהתאם"; 340=34/יח' — הוצג לו כניתן-לשינוי-בדקה). מוסדות 2,000→**2,500** (מינימום 25 ש"ח/יח'), FAQ רוכך "אפשר לתאם כל כמות". **מתנת מפיצים = ספר קהלת לקראת סוכות** (לא שופטים! שופטים = מתנת הפרק-השבועי בלבד).
7. Snapshot לפני: `campaigns_bak_20260915b_dor` + tiers (RLS). אומת חי: dump-dom (כל המחרוזות, אפס "אקוורל"/"קמפיין"), ₪200/₪340 בבנדל, צילומי CDP נייד+דסקטופ.
8. **נקודות איסוף (הקלטת-הקבוצה 09:57):** ליואב יש ~5 נקודות קיימות; סוכם עקרונית (בתשובה בפרטי) שכשישלח רשימה בכתב — מעלים לדף + שדה בחירת-נקודה בטופס ההרשמה של הפרק השבועי. ⬜ ממתין לרשימה.
9. תשובה מסכמת נשלחה ליואב בפרטי מקו 1 (`3EB0C9E9FE7A3B79CA7B3D`, delivered, נרשם בשער).

### 15.9.2026 אחה"צ — בחירת נקודת-איסוף לספר המתנה בהרשמת הפרק השבועי (`c4d58cb5`, פרוס ואומת E2E)

בקשת סער: הנרשם בוחר נקודת איסוף לספר שופטים בתוך ההרשמה, נשמר בדאטה בשליפה קלה, והרשימה מאומתת מול הנקודות שקיימות באתר.
1. **מקור-אמת יחיד — `sale_points` הקיימת** (מנוהלת מ-/admin מאז יואב 13.7; RLS ציבורי ל-is_active). לא נוצרה רשימה כפולה. כרגע נקודה פעילה אחת: "איסוף עצמי - ירושלים, קרית מנחם". כשהרשימה של יואב תגיע — מוסיפים באדמין וזה מתעדכן בכל שלושת המקומות בלי דיפלוי (בורר-ההרשמה, שורת הדף, החנות).
2. **QuickBuyDialog:** prop `pickupPointLabel` — בורר חובה מ-usePublicSalePoints; מוסתר כשאין נקודות פעילות; מציג איש-קשר של הנקודה. SubscribeButton (הפרק-השבועי) מדליק אותו.
...
### 15.9.2026 ערב — סבב שיחת הרב יואב: חבילות מ-5, הירו-תמונה, אנשי קשר פרטיים (`087c4895`, **תצוגה מקדימה בלבד**)

מקור: הקלטת שיחה סער↔יואב (15.9 18:09, צ'אט-עצמי, תומללה ב-whisper-cli) + 2 הקלטות יואב במסתערים (16:21/16:26) + רשימת נקודות ואנשי קשר שיואב כתב במסתערים (13:14, 14:34).
1. **הכרעות השיחה:** מוכרים בשלב זה רק מ-5 חוברות ומעלה, משלוח כלול (ברירת מחדל), "מהדורה מוגבלת" בכותרת, בלי הזמנה לפתוח נקודות מכירה (גל שני אולי); הנקודות הקיימות נשארות כאפשרות איסוף, וכל מי שבוחר נקודה מקבל את איש הקשר שלה. 5=197 (יואב). 10/20/50/100 = **הצעה** 347/647/1,497/2,497 (סיומת 7, רצפת מוסדות 25/יח') — ממתין לאישור.
2. **תצוגה בלי לגעת בפרודקשן:** קמפיין-שכפול `dor-haplaot-preview` (show_site_banner=false, sort 999, page_code_env=PRODUCTS) עם 5 החבילות החדשות, סיפור בלי קופסת "רשת המפיצים", FAQ מעודכן. קוד: build מקומי → פרויקט Vercel נפרד `bz-dor-preview` (dist + vercel.json rewrite). ה-API בתצוגה לא פעיל (אין env) — תשלום נבדק רק בפרוד.
3. **הירו מצב-מוצר:** התמונה עצמה היא ההירו — ברוחב מלא, בהירות רגילה, כותרות מעל, תיאור+CTA מתחת (דפוס Atomic Habits מסקירת אתרי ספרים). בנייד חיתוך 4:5 סביב החוברת. אומת בצילום CDP מובייל.
4. **צ'קאאוט:** בחבילה עם משלוח כלול — בחירה "משלוח עד הבית / איסוף עצמי"; באיסוף — בורר נקודה, בלי איש קשר (רק "יישלחו במייל").
...
### 15.9.2026 לילה — סבב המשך (סער: "את השינויים תפרוס שאתה מסיים לעבוד")
1. **דור הפלאות — חבילות 1-2 חזרו** לפי הקלטת יואב 18:44 ("אפשר לקנות מאחד ומעלה, משלוח הביתה רק מחמש ומעלה"): tier-1 (44) ו-tier-2 (80) פעילים, `needs_shipping=false` = איסוף עצמי חינם, בורר נקודה חובה. 5+ ללא שינוי. FAQ עודכן. snapshot: `campaigns_bak_20260915d_dor` + tiers. DB-only, אומת חי (7 חבילות, חלון חוברת-אחת עם 8 נקודות).
2. **דף הפרק השבועי (יהושע):** המשפט "אוספים את הספר מנקודת איסוף. נקודות איסוף: איסוף עצמי - ..." → "אוספים את הספר מאחת מנקודות האיסוף: ...". הנקודה "איסוף עצמי - ירושלים, קרית מנחם" שונתה ל-"ירושלים, קרית מנחם" (אחידה עם השאר).
3. **תקלת העלאה של יוצר-תוכן (הרב שלמה מונדשיין, shlomo6963):** לוג Postgres 18:41 = `new row violates row-level security policy for table "series"`. שורש: מאז רמה 14 (`admin_write_level14`) ל-creator אין כתיבה ל-lessons/series/lesson_topics/lesson_rabbis בכלל — כל "שלח לאישור" נחסם. **לא נפתחו הרשאות** (מודל-אבטחה = אישור נפרד); הוסבר ליואב ונשאל (`3EB0FD8E5E3F9A2E3136C3`, delivered). הצעה: פוליסי צרה — INSERT בסטטוס draft/pending_review על שמם בלבד. בקוד: הודעת השגיאה מציגה עכשיו את הסיבה (שגיאת Supabase היא אובייקט, לא Error → "שגיאה לא ידועה").

### 15.9.2026 לילה — יוצרי תוכן מפרסמים ישירות (אפשרות א' של הרב יואב; סער: "הוא רוצה אופציה א קדימה לעבודה")
הרקע: הרב שלמה מונדשיין (creator) נחסם בהעלאה — מאז רמה 14 ל-creator אין כתיבה בכלל. יואב קיבל הסבר על חומרת פרסום ישיר (`3EB0FE05985D986178E183`) ובחר א': פרסום ישיר עם גבולות.
1. **RLS (אדיטיבי, רק ל-creator, ה-admin_write_level14 לא נגע):** `lessons.creator_insert_own` / `creator_update_own` — רק `submitted_by = auth.uid()`, סטטוס draft/pending_review/published; `series.creator_insert_own` / `creator_update_own` — רק על שמם, draft/active; `lesson_topics` + `lesson_rabbis.creator_link_own_lesson` — קישור רק לשיעור שלהם. **אין DELETE**, אין נגיעה בתוכן של אחרים. גיבוי הפוליסיז הקודמות: scratchpad `policies_before_creator.json`.
...
- **המשך (סער: "תן משהו עוצמתי ברקע... ובאותיות אפקט זהב מנצנץ כמו בראשי"):** רקע מג'ונרט Nano Banana Pro (`gemini-3-pro-image`, 2752×1536; ⚠️ `gemini_image.py` נופל על TLS של פייתון 3.14 — בנייה ב-פייתון ושליחה ב-curl) → `v3/hero-bg-sunrise.webp` (זריחה, קרני אור, גבעות, זית וחיטה; חלופה `v3/hero-bg-jerusalem.webp`). שכבת שמנת מימין (צד הטקסט) בדסקטופ, ומלמעלה/למטה בנייד. הכותרת: `animate-shimmer` של דף הבית עם גרדיאנט זהב עמוק יותר (הזהב החיוור של דף הבית לא קריא על רקע בהיר). ⬜ "פרוס".
- **המשך (סער: "תגנרט מחדש את החוברת, לא נראית טוב בקצוות"):** החוברת נוצרה מחדש ב-Nano Banana Pro עם הכריכה המקורית מקובץ הדפוס (`pdftoppm` עמוד 1, 200dpi) כ-inline_data + סעיף FIDELITY, על רקע סטודיו לבן, softcover עם שדרה משמאל (וריאנט כריכה-קשה נפסל — מטעה לחוברת). חיתוך `rembg -a -ae 12` (alpha matting) → `v4/book.webp` 1100×1672 (פי 2 מהקודם). קצוות, שדרה והכיתוב אומתו בזום. DB-only: `campaigns.hero_product_image_url`. ⬜ "פרוס".

### 15.9.2026 לילה — פרשת השבוע הציגה "שלח לך" + מועד שאחרי + ולידציית טלפון (פרוס `5dd3297a`, אומת חי)
סער: "תיקון חמור — למה באתר הפרשה המעודכנת היא שלח לך?" + "יש מועד קרוב יום כיפור ואחכ סוכות" + אישור מיזוג תיקון הטלפון מ-23.8.
1. **שורש הפרשה:** `getCurrentParasha` (hebcal, Israel) תקין, אבל hebcal v6 מאיית `Parashat Ha'azinu` ו-`Parashat Nasso` והמפה `EN_TO_HE` ציפתה ל-`Ha'Azinu` / `Naso` → החיפוש נכשל → הקוד החזיר "שלח לך" **קבוע** (גם ב-fallback). בדיקה של כל שבתות תשפ"ו–תשפ"ח: בדיוק 2 שמות חסרים. תוקן: שני המפתחות, חיפוש לא-רגיש-לאותיות (`lookupHe`), במקום "שלח לך" קבוע — `ev.render("he-x-NoNikud")`, בדיקת דגל ב-bitmask. שבועות חג (יום כיפור/סוכות) — אין פרשה ב-9 ימים → חיפוש הפרשה הבאה עד 45 יום (במקום טבלה סטטית של 2026). בדיקת תאריכים: 13–19.9.26 האזינו · 20.9–10.10 בראשית · 11.10 נח; נשא 20.5.26 ✓.
2. **המועד:** דף הבית (`DesignPreviewHome` = Index) כבר הציג נכון "יום כיפור, עוד 6 ימים" מ-`lib/holidays.ts`. נוספה שורה "ואחריו: סוכות · ט״ו בתשרי · עוד 11 ימים" (`getUpcomingHolidays()[1]` בתוך 30 יום).
3. **ולידציית טלפון (`a6e42e0f` מ-23.8, cherry-pick נקי מעל נקודות האיסוף):** נרמול +972/רווחים → 05XXXXXXXX בלקוח ובשרת, דחייה ברורה לפני Grow (שגיאה 717), דחיית Grow נרשמת כ-failed. אומת חי: create-payment עם "12345" → 400 "נא להזין נייד ישראלי בן 10 ספרות" (לפני יצירת שורה).
4. ⚠️ שגיאות tsc ב-`api/grow/create-payment.ts` שורות 530/584 (טיפוס SupabaseClient ל-computeDedicationPrice) — קיימות גם ב-main לפני המיזוג, לא חוסמות build של Vercel.

### 16.9.2026 — התראות ההרשמה לפרק השבועי: נמען יחיד שלא נצפה + אפס תיעוד (פרוס `8edbe882`)

**איך זה התגלה:** אביה שאל בקבוצת התפעול למה הגיעה מצטרפת חדשה (שרי לורך, 16.9 08:05, אסמכתא 517333313) בלי מייל "מצטרף חדש" מהאתר, ושאל אם היא בכלל נרשמה דרך האתר.

**האבחון — ההרשמה עצמה עבדה במלואה:** הזמנה 08:05:02 (`order_number` ריק = צ'קאאוט מהאתר; מי שמגיע מייבוא-הפיד של Grow מקבל `GROW-xxx`) → `user_access_tags` 08:06:05 מ-`grow_webhook` → `smoove_subscribed=true` → פריט מאנדיי `3226853487` ב-🟢 פעילים 08:06:12. הכשל היחיד היה בשכבת ההתראות.

**שני הליקויים:**
1. ההתראה יצאה ל-`office@bneyzion.co.il` **בלבד** — תיבה שאיש לא צופה בה בפועל.
2. `sendSingleEmail` הייתה fire-and-forget. אחרי מעשה אי אפשר היה לענות "יצא המייל או לא" — לא בלוגים (runtime logs של ורסל לא נגישים ב-API), לא ב-DB, ולא ב-Smoove (**אין GET על `/Campaigns` — כל ניסיון מחזיר 405**, גם על קמפיין של החשבון שלנו).

**מה שנבדק לפני שינוי הקוד:** שחזור מלא של מסלול השליחה מול Smoove — נמען יחיד (קמפיין `55556000`) ושני נמענים בקמפיין אחד (`55556025`), שניהם HTTP 200 עם `id`. כלומר המפתח וה-API תקינים; הבעיה הייתה הנמען וחוסר העקבה. ראיה נוספת שהקוד כן הגיע לשלב המייל באותו בוקר: איש הקשר `office@bneyzion.co.il` בסמוב נושא `lastChanged = 16.9 08:06:06` — ה-upsert שקודם לקמפיין.

**התיקון:**
- `sendEmailToRecipients()` ב-`api/lib/digital-delivery.ts` — נמענים מרובים ב**קמפיין אחד** (לחשבון Smoove יש תקרת-קמפיינים יומית שנחסמת בשקט; קמפיין-לכל-נמען שורף אותה פי-כמה), מחזירה `{ok, campaignId, error, at, to}`. `sendSingleEmail` נשמרה כעטיפה — 7 הקוראים האחרים לא נגעו.
- נמעני ההתראה: `office@bneyzion.co.il` + אביה (`tchvesua41@gmail.com`), ניתנים לשינוי מ-Vercel בלי דיפלוי דרך `OFFICE_NOTIFY_EMAILS`.
- **`orders.notify_log` (jsonb, נוסף חי)** — כל תוצאות ההרשמה: התראת המשרד, מייל נקודת האיסוף, והמאנדיי. ⚠️ **בלי כתובות הנמענים** — `orders_owner_read` נותנת לבעל ההזמנה לקרוא את השורה שלו, ומייל פרטי של איש-צוות לא נכנס לעמודה כזו.
- `sendPickupConfirmationForPoint` מחזירה עכשיו `boolean` כדי שהתוצאה תיכנס לפנקס.

**אימות:** `tsc -b` נקי; דיפלוי `dpl_45rRopZYsXSmcLVcg2MDCXTbpjAD` READY; `GET /api/grow/webhook` → 405 ו-`/api/admin/creator-publications-digest` → 401 (הפונקציות נטענות — מלכודת ה-ESM לא נפתחה), דף הבית 200. ⬜ **אימות E2E אמיתי יגיע עם ההרשמה הבאה** — לבדוק `notify_log` על השורה החדשה.

**לקח כללי:** התראה תפעולית לנמען יחיד ובלי רישום-תוצאה היא נקודת-כשל שקטה כפולה — גם לא מגיעה, וגם לא ניתנת לבדיקה בדיעבד.

### 15.9.2026 לילה — סבב תיקוני הרב יואב והרב מונדשיין (פרוס `c04e339f`, אומת חי)
1. **קישור שם-הרב → דף כל הרבנים:** `DesignPreviewSeriesPageV2` בנה `/rabbis/${lesson.rabbis?.id || ""}`, והשאילתות (`useSeriesMixedContent` ועוד 10) בחרו `rabbis!lessons_rabbi_id_fkey(name)` בלי id → `/rabbis/`. נוסף `id` בכל השאילתות, ובלי id — שם בלי קישור.
2. **`/rabbis` (RabbisList.tsx, הדף החי — לא DesignPreviewRabbisList):** שורת חיפוש לפי שם (מתעלם מגרשיים). אומת: 149 → 2 על "מונדשיין".
3. **"השיעור לא מופיע בדף שלי":** ל-160 רבנים יש `rabbi_page_items` (רשימה מסודרת 1:1 מהאתר הישן) ו-RabbiPage מציג רק אותה. עכשיו: סדרות ושיעורים-עצמאיים שנוצרו אחרי הפריט האחרון ברשימה מוצגים בראש הדף; הסדר הקיים לא נוגע. hooks הוסיפו `created_at`/`series_id`.
4. **"האזן" במקום "קרא":** האשף שמר `source_type = form.sourceType` (ברירת-מחדל audio) גם למאמר טקסט. עכשיו נגזר מהתוכן בפועל (video/audio/text/document). השיעור `bb843f21` תוקן ל-text ב-DB.
5. **נקודות איסוף:** עלי = אביה 0527471094, פתח תקווה = אוי 0524046006 (sale_point_contacts).
6. ⬜ **"תמונות עם אותיות" (יואב):** בסריקת דף הבית אין תמונה עם אותיות ברורות; המועמדות — איורי ספר/מגילה פתוחים עם שורות-טקסט מדומות (כרטיסי שיעורים, כרטיס הפרשה). ממתין לדוגמה מיואב.

...
### 2026-05-27 — Phase 2 attachment migration — relative paths + empty strings (script `migrate_phase2.py`)

**מה עשינו:**
- Sub-task D: 71 רשומות עם `attachment_url = ''` → `NULL`. שמרנו `legacy_attachment_url = ''` (איבחון).
- Sub-task A: 3,035 רשומות עם relative paths (`/media/...`) → Supabase URLs.
  - 307 distinct paths מתוך 318 כבר קיימים ב-Supabase (match ע"י `legacy_attachment_url`). **2,906 רשומות = DB UPDATE בלבד — אפס uploads מיותרים.**
  - 11 distinct paths חדשים → 11 uploads לחנות → 129 rows מעודכנות.
  - Total: 3,035 rows, 0 errors, ~80 שניות.
- Sub-task B: 3 רשומות עם `https://www.bneyzion.co.il/media/...` + `legacy_attachment_url IS NULL` → **כולן 404 קבוע** (קבצים נמחקו מ-Umbraco). נשאר `bneyzion_media_count = 3` עד החלטת סאר.

**DB state לאחר Phase 2:**
```
null_count:    ~11,105 (כולל 71 שנוספו מ-D)
supabase_count: ~7,782 (היה 4,709 לפני Phase 2)
bneyzion_count: ~1,020 (כולם page URLs — Sub-task C)
relative_count: 0 ✅
empty_count:    0 ✅
other_count:    0 ✅
```

**Sub-task C — ממתין להחלטת סאר:**
1,020 רשומות עם `attachment_url` שמצביע לדף Umbraco (כגון `https://www.bneyzion.co.il/מאגר-השיעורים-והמאמרים/...`), לא לקובץ ישיר.
- הדפים מחזירים 301→200 עם תוכן HTML.
- **כל דף מכיל מספר קישורי `/media/...` מוטמעים** (אוסף של קבצים, לא קובץ בודד).
- Option C1: לסרוק כל דף, לחלץ את קישורי ה-media המוטמעים, לעדכן ל-Supabase URL של הקובץ הראשי.
- Option C2: `attachment_url = NULL, legacy_attachment_url = old_url` — ה-modal לא יציג כפתור PDF. ניתן לחזור אחר כך.
- **המלצה: C2** (הדפים הם אוספי קבצים, לא 1:1 עם שיעור).

**קבצים:**
- `scripts/migrate_phase2.py` — script החדש (תומך `--task D|A|B|ALL`, `--dry-run`, `--workers N`)
- `scripts/migrate_phase2_state.json` — state file עם upload_cache + done_a + errors_b

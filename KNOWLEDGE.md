...
#### Iteration 2 (same day, Saar feedback) — commit `3f0d8d31`
- **New nav category "הקורסים שלי" → `/design-my-courses`** (added to `NAV_ITEMS` between פרשת השבוע and חנות; auto-appears in mobile menu too). ⚠️ Saar explicitly chose `/design-my-courses` (the dedicated personal-learning page, auth-gated) over `/courses` or `/portal` — the label "הקורסים שלי" is inconsistent in the codebase (UserMenu→`/portal`, PortalSubscriber→`/courses`, dedicated page→`/design-my-courses`).
- **Categories row nudged** right for aesthetics: `transform: translateX(22px)` on the desktop `<nav>` (physical px — translateX+ = toward logo in RTL; no reflow). ⚠️ First tried `translate(22px,6px)` but the +6px Y dropped the row below the התחברות button — Saar: "הורדת נמוך מידי וזה לא מיושר בגובה עם ההתחברות". Removed the Y; nav link center now matches login button center (both mid=48px @1440). Commit `8b306915`.
- ⚠️ Claude_Preview's headless window is ~227px wide → desktop nav (`hidden md:flex`) is `display:none` there; verify desktop header via playwright at ≥1280px (system Chrome, `playwright-core` is in node_modules). Verified at 1440px: nav order + nudge correct, cream-warm intact on both transparent (home) and parchment (/about) variants.
- **Open question to Saar (pending):** search modal *interior* uses `--popover: 0 0% 100%` (pure white) — design-system default, same as the old Header always looked. Offered a scoped cream-warm tint (GlobalSearch only, not the global popover token); awaiting decision.

#### Iteration 3 (same day, Saar feedback on the live modal) — commit `0e9815f5`
- Saar: "מעוצב מעולה, רק האיקס בשורת חיפוש להזיז לצד שני" + "תגיות עם נושאים.. לא מוביל לשום מקום".
- **Close (X) → top-LEFT for RTL:** the shadcn `DialogContent` close button is hard-coded `absolute right-4 top-4` (dialog.tsx, LTR default). Overrode **only** in `command.tsx` `CommandDialog`'s `DialogContent` className: `[&>button.absolute]:left-4 [&>button.absolute]:right-auto`. `CommandDialog` is used by GlobalSearch ONLY → scoped, doesn't touch other dialogs. Verified computed `left:16px` at both mobile and 1440px.
- **Topics (נושאים) removed entirely:** they linked to `/series?topic={slug}` but there is **no `/series` route and no topic-filter route anywhere** (only `/series/:id`) → dead link. Removed from `useGlobalSearch` (TopicResult, query, state, suggestions, totalResults — also dropped now-unused `mainPattern`) and from `GlobalSearch` render (+ unused `Tag` import, + suggestion badge "נושא"). Final search scope = **סדרות · שיעורים · ספרים · רבנים** (matches Saar's original ask).
- ⚠️ `playwright-core` headless (system Chrome) renders the page but **cannot paint the cmdk CommandDialog content** (blank cream card) — use the Claude_Preview MCP browser for modal screenshots, or verify via DOM eval (`[cmdk-group-heading]` list + close-button computed `left`). The site also has a floating AI-chat widget with `role="dialog"` — when querying the search dialog, anchor on `[cmdk-root].closest('[role=dialog]')`, not the first `[role=dialog]`.

#### Iteration 4 — SHIPPED TO LIVE (Saar approved 2026-06-01)
- **`vercel --prod --yes`** from `bneyzion-header` worktree → production deployment `dpl_43txWASLmjY3guHytMkcwxz5XcSn`, `target:production`, aliased to **bneyzion.vercel.app** (new bundle `main-D_je1SNF.js`). Verified live in real browser: nav = אגף המורים·פרשת השבוע·הקורסים שלי·חנות·אודותינו, search present, no dark toggle.

...
### 2026-05-27 — Media recovery: 11 lessons MEDIA_404 + scanner analysis

**A. MEDIA_404 — embedded Umbraco images in lesson content HTML**
- הבעיה: 11 שיעורים כללו `<img src="/media/144XXX/...">` ב-content HTML — URL יחסי שהולך ל-Vercel (שאין לו /media/ route) במקום ל-Umbraco
- הקבצים: 4 תמונות ייחודיות (לא PDFs!) — ציר זמן, מסעות הארבעים, נדודי דוד, משפחת ירמיה
- Recovery: הורדה מ-Umbraco (www.bneyzion.co.il/media/...) + upload ל-Supabase Storage `lesson-images/legacy-recovery/`
- תוקן: REGEXP_REPLACE ב-content HTML, 11 שיעורים × 4 UPDATEs
- גם הוסרו 8 hidden `<img src="/umbraco/nothing.jpg" style="0x0px">` Umbraco artifacts
- Rollback: `audits/sql-fixes-2026-05-27-rollback.sql` FIX 4
- Full report: `audits/recovery-2026-05-27.md`

**B. NO_H1 (28) + TIMEOUT (1) = timing artifacts**
- כל 28 השיעורים עם NO_H1 תקינים לחלוטין. Screenshot מאשר h1, rabbi, series, audio — הכל מוצג
- root cause: scanner `wait_for_selector("h1", timeout=8000)` — 8 שניות. כל ה-28 הרנדרו ב-~8200ms (cold cache)
- fix לסקאנר: שנה timeout ל-15000 ב-`scripts/playwright-lessons-scan.py` שורה 45
- TIMEOUT (ויקרא מוקלט פרק ח): cold start של Vercel בזמן 12K parallel requests בסריקה

**Iron rules learned:**
- אסור לסמוך על "MEDIA_404" בלי לבדוק את ה-content HTML — הבעיה יכולה להיות img embedded, לא PDF
- `render_ms ~8200` עם `wait_for_selector timeout=8000` = timing artifact, לא באג
- Umbraco `/umbraco/nothing.jpg` = placeholder 0x0 שמופיע ב-content HTML של כל שיעור עם תמונה מ-Umbraco v7+

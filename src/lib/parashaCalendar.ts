...
/**
 * Known parasha schedule for 5786 (2025-2026) — Israel reading cycle
 * Verified against hebcal.com/shabbat with i=on (Israel mode).
 * Note: In Israel several double-parshiot are read that Diaspora reads separately.
...
const SCHEDULE_5786: Array<[number, number, string]> = [
  ...
  [1, 14, "משפטים"],
  [1, 21, "תרומה"],
  [1, 28, "תצוה"],    // Feb 28 2026
  [2, 7, "כי תשא"],  // Mar 7 2026
  [2, 14, "ויקהל"],  // Mar 14 — ויקהל-פקודי in Israel (double)
  [2, 21, "ויקרא"],  // Mar 21 (was "פקודי" — wrong)
  [2, 28, "צו"],     // Mar 28 (was "ויקרא" — wrong)
  // Pesach: no regular reading Apr 4
  [3, 11, "שמיני"],  // Apr 11 (was [3,18] — wrong)
  [3, 18, "תזריע"],  // Apr 18 — תזריע-מצורע in Israel (double)
  [3, 25, "אחרי מות"], // Apr 25 — אחרי מות-קדושים in Israel (double)
  [4, 2, "אמור"],    // May 2 (was [4,23] — wrong, off by 3 weeks)
  [4, 9, "בהר"],     // May 9 — בהר-בחוקותי in Israel (double)
  [4, 16, "במדבר"],  // May 16
  [4, 23, "נשא"],    // May 23
  [4, 30, "בהעלותך"], // May 30 (was "בהר" — WRONG, caused the reported bug)
  [5, 6, "שלח לך"],  // Jun 6 (was "בחוקותי" — wrong)
  [5, 13, "קורח"],   // Jun 13 (was "במדבר" — wrong)
  [5, 20, "חוקת"],   // Jun 20 (was "נשא" — wrong)
  [5, 27, "בלק"],    // Jun 27 (was "בהעלותך" — wrong)
  [6, 4, "פנחס"],    // Jul 4 (was "שלח לך" — wrong)
  [6, 11, "מטות"],   // Jul 11 — מטות-מסעי in Israel (double)
  [6, 18, "דברים"],  // Jul 18 (was "קורח" — wrong)
  [6, 25, "ואתחנן"], // Jul 25 (was "חוקת" — wrong)
  [7, 1, "עקב"],     // Aug 1 (was [7,2,"בלק"] — wrong)
  [7, 8, "ראה"],     // Aug 8 (was [7,9,"פנחס"] — wrong)
  [7, 15, "שופטים"], // Aug 15 (was [7,16,"מטות"] — wrong)
  [7, 22, "כי תצא"], // Aug 22 (was [7,23,"מסעי"] — wrong)
  [7, 29, "כי תבוא"], // Aug 29 (was [7,30,"דברים"] — wrong)
  [8, 5, "נצבים"],   // Sep 5 — נצבים-וילך in Israel (double)
  [8, 19, "האזינו"], // Sep 19 — Shabbat Shuva
];

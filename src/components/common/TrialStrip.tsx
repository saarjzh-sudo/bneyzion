/**
 * TrialStrip — רצועת "האתר בהרצה" + דיווח תקלות (רמה 27, יואב 22.7 16:31).
 *
 * רצועה דקה מתחת להדר. שני מסלולי דיווח (כיוון סער 17:49):
 *   1. וואטסאפ מהיר לסער — עם כתובת העמוד הנוכחי, לטיפול בזמן אמת.
 *   2. טופס קצר → טבלת site_feedback → נקרא ב-/admin/feedback.
 *
    ...
    <>
      ...
      >
        ...
        <button
          ...
          style={{ background: "rgba(232,213,160,0.16)", border: "1px solid rgba(232,213,160,0.4)", color: "#E8D5A0", borderRadius: 999, padding: "0.15rem 0.85rem", fontFamily: fonts.body, fontSize: "0.75rem", fontWeight: 700, cursor: "pointer" }}
        >
          דיווח על תקלה
        </button>
      </div>
      ...
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="דיווח על תקלה או הערה"
          ...
          <div style={{ background: "white", borderRadius: radii.xl, width: "min(440px, 96vw)", padding: "1.5rem", boxShadow: "0 24px 70px rgba(0,0,0,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
              <h2 style={{ fontFamily: fonts.display, fontWeight: 800, fontSize: "1.1rem", color: colors.textDark, margin: 0 }}>
                נתקלתם בתקלה? יש הערה?
              </h2>
              <button onClick={() => setOpen(false)} aria-label="סגירה" style={{ background: "none", border: "none", cursor: "pointer", color: colors.textMuted }}>
                <X size={18} />
              </button>
            </div>
            ...
            >
              <MessageCircle size={16} /> דיווח מהיר בוואטסאפ
            </a>
            ...
            {state === "sent" ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                ...
                >
                  <Send size={14} /> {state === "sending" ? "שולח…" : "שליחת הדיווח"}
                </button>
                {state === "error" && (
                  <div style={{ fontFamily: fonts.body, fontSize: "0.78rem", color: "#A33", textAlign: "center" }}>
                    השליחה נכשלה — אפשר לדווח בוואטסאפ למעלה
                  </div>
                )}
              </div>
            )}

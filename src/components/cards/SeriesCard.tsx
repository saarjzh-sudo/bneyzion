...
interface SeriesCardProps {
  ...
  title: string;
  lessonCount: number;
  /** Single rabbi name (legacy callers). Superseded by rabbiLabel when provided. */
  rabbiName?: string | null;
  /**
   * Pre-formatted multi-rabbi label (C6 — "X, Y ועוד").
   * When provided, renders instead of rabbiName.
   */
  rabbiLabel?: string | null;
  imageUrl?: string | null;
  onClick?: () => void;
}

const SeriesCard = memo(function SeriesCard({ id, title, lessonCount, rabbiName, rabbiLabel, imageUrl, onClick }: SeriesCardProps) {
  const displayRabbi = rabbiLabel ?? rabbiName ?? null;
  const content = (
      ...
      <div className="relative z-10 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1.5 text-xs text-muted-foreground">
            {displayRabbi && <span>{displayRabbi}</span>}
            {displayRabbi && lessonCount > 0 && <span className="text-border">·</span>}
            {lessonCount > 0 && (

import os, sys, json, subprocess, glob, urllib.parse
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = "/tmp/rverify"; os.makedirs(OUT, exist_ok=True)
cands = glob.glob(os.path.expanduser("~/.cache/puppeteer/chrome-headless-shell/*/chrome-headless-shell-*/chrome-headless-shell"))
CHROME = cands[0] if cands else "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
def noenv():
    e = dict(os.environ)
    for k in ("HTTP_PROXY","HTTPS_PROXY","http_proxy","https_proxy"): e.pop(k, None)
    e["NO_PROXY"]="*"; e["CHROME_BIN"]=CHROME; return e
man = {m["name"]: m for m in json.load(open(os.path.join(HERE, "rabbi-manifest.json"), encoding="utf-8"))}
old = json.load(open(os.path.join(HERE, "oneone/old_rabbi_pages.json"), encoding="utf-8"))
SAMPLE = ["הרב יואב אוריאל","הרב איתן שנדורפי","הרב מנחם שחור","הרב יוסי ברינר",
          "הרבנית דנה סליי (לנשים)","הרב עמנואל בן ארצי","הרב אברהם וסרמן","הרב יוסף שילר",
          "הרב אליעזר קשתיאל","הרב נועם וידר"]
NEW = "https://bneyzion.vercel.app"
def shot(url, path):
    try:
        subprocess.run(["node", os.path.join(HERE,"fullshot.cjs"), url, path, "1366"],
                       capture_output=True, text=True, env=noenv(), timeout=160)
        return os.path.exists(path) and os.path.getsize(path) > 5000
    except Exception: return False
for i, name in enumerate(SAMPLE, 1):
    m = man.get(name)
    if not m or not m.get("slug"): print(f"[{i}] {name}: no slug/map"); continue
    safe = name.replace("/", "_").replace('"','').replace(" ","_")
    nu = f"{NEW}/rabbis/{m['slug']}"
    ou = old.get(name, {}).get("url") or (f"https://www.bneyzion.co.il/מאגר-השיעורים-והמאמרים/רבנים?rav=" + urllib.parse.quote(name))
    n = shot(nu, f"{OUT}/{safe}-new.png"); o = shot(ou, f"{OUT}/{safe}-old.png")
    print(f"[{i}/{len(SAMPLE)}] {name[:24]:24} new={'OK' if n else 'X'} old={'OK' if o else 'X'} | old_items={m['old_items']} rpi={m['cur_rpi']} slug={m['slug']}", flush=True)
print("DONE", flush=True)
